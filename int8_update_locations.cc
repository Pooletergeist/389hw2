...
std::vector<int8_t> generate_random_list(int size, int bound) {
    std::vector<int8_t> vec (size);
    for (int i=0; i<size; i++) {
        vec[i] = (int8_t) (rand() % bound) - bound + 1;
    }
    return vec;
}
...
void update_coords (
    std::vector<int8_t> &xs, std::vector<int8_t> &ys, 
    std::vector<int8_t> &zs, const std::vector<int8_t> &vx,
    const std::vector<int8_t> &vy, const std::vector<int8_t> &vz
    ) {
...
int main (int argc, char* argv[]) {
   if (argc != 3) {
    ...
    std::vector<int8_t> xs = generate_random_list(size, 1000.);
    std::vector<int8_t>ys = generate_random_list(size, 1000.);
    std::vector<int8_t>zs = generate_random_list(size, 1000.);

    std::vector<int8_t>vx = generate_random_list(size, 1.);
    std::vector<int8_t>vy = generate_random_list(size, 1.);
    std::vector<int8_t>vz = generate_random_list(size, 1.);
